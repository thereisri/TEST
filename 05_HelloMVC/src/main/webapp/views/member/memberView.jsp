			...
			<table>
				...
				<tr>
					...
					<td>	
						...
						name="address" id="address" value="<%=m.getAddress()%>"><br>
					</td>
				</tr>
				<tr>
					<th>성별 </th>
					<td>
							...
							value="M" <%=m.getGender()=='M'?"checked":"" %>>
							<label for="gender0">남</label>
							<input type="radio" name="gender" id="gender1" 
							value="F" <%=m.getGender()=='F'?"checked":"" %>>
	...
	<script>
		...
		const fn_updateMember=()=>{
			$("#memberFrm").attr("action",
					"<%=request.getContextPath()%>/member/updateMember.do");
			$("#memberFrm").submit();
		...
		const fn_deleteMember=()=>{
			$("#memberFrm").attr("action",
				"<%=request.getContextPath()%>/member/deleteMember.do");
			$("#memberFrm").submit();
		}
	</script>
